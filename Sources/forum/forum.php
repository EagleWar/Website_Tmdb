...
<html>
  ...
  <body>
    ...
    if (isset($_GET['title']) && $_GET['title'] != ""  && $_GET['title'] != " " && isset($_GET['add'])) {


          $title = $_GET['title'];
          $search = "%".$title."%";

          $sql_rows = "SELECT * FROM forum WHERE title like'$search' ";
          ...
          $sql = "SELECT * FROM forum WHERE title like'$search' LIMIT $begin, $row_max ";
          ...
          if (mysqli_num_rows($result) > 0 ) {
                        ...
                        <div style="margin:auto;" class="row">
                            <table class="print-table" >
                              <tr >
                                ...
                                <td class="creator"> <?php  echo "created by ". $row["creator"];?> </td>
                              </tr>
                            </table>
            ...
            ?> <h3>There is no subject for this research</h3>
            <?php
          }

          }
  else {
    ?>  <h1>Last forums</h1>  <?php


    $sql_rows = "SELECT * FROM forum LIMIT 100 ";
    $result = mysqli_query($conn, $sql_rows);
    $num_rows = mysqli_num_rows($result);
    ...
    $sql = "SELECT * FROM forum LIMIT $begin, $row_max  ";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0 ) {
            while($row = mysqli_fetch_assoc($result)){
            ?>
  ...
  if ($num_rows > $row_per_page) {
    ...
    ?>   <a href="forum.php?page=<?php echo "$i";?>"> <?php echo "$i";?></a> <?php
