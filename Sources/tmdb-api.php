...
class TMDB {
	 ...
	 * 	@return Movie[]
	 */
	public function getNowPlayingMovies($page = 1, $language) {

		$movies = array();

		$result = $this->_call3('movie/now_playing', '&page='. $page, $language);
	 ...
	 * 	@return Movie[]
	 */
	public function getPopularMovies($page = 1, $language) {

		$movies = array();

		$result = $this->_call3('movie/popular', '&page='. $page, $language);
	 ...
	 * 	@return Movie[]
	 */
	public function getTopRatedMovies($page = 1, $language) {

		$movies = array();

		$result = $this->_call3('movie/top_rated', '&page='. $page, $language);
	 ...
	 * 	@return Movie[]
	 */
	public function getUpcomingMovies($page = 1, $language) {

		$movies = array();

		$result = $this->_call3('movie/upcoming', '&page='. $page, $language);
	 ...
	 * 	@return TVShow
	 */
	public function getLatestTVShow($page = 1,$language) {
		return new TVShow($this->_call('tv/latest','&page='. $page,$language));
	}
	 ...
	 * 	@return TVShow[]
	 */
	public function getOnTheAirTVShows($page = 1,$language) {

		$tvShows = array();

		$result = $this->_call3('tv/on_the_air', '&page='. $page,$language);
	 ...
	 * 	@return TVShow[]
	 */
	public function getAiringTodayTVShows($page = 1, $timeZone = null, $language) {
		$timeZone = (isset($timeZone)) ? $timeZone : $this->getConfig()->getTimeZone();
		$tvShows = array();

		$result = $this->_call('tv/airing_today', '&page='. $page, $language);
	 ...
	 * 	@return TVShow[]
	 */
	public function getTopRatedTVShows($page = 1, $language) {

		$tvShows = array();

		$result = $this->_call('tv/top_rated', '&page='. $page, $language);
	 ...
	 * 	@return TVShow[]
	 */
	public function getPopularTVShows($page = 1, $language) {

		$tvShows = array();

		$result = $this->_call3('tv/popular', '&page='. $page, $language);
	 ...
	 * 	@return Movie
	 */
	public function getMovie($idMovie, $appendToResponse = null, $language) {
		$appendToResponse = (isset($appendToResponse)) ? $appendToResponse : $this->getConfig()->getAppender('movie');

		return new Movie($this->_call3('movie/' . $idMovie, $appendToResponse, $language));
	}
	 ...
	 * 	@return TVShow
	 */
	public function getTVShow($idTVShow, $appendToResponse = null, $language) {
		$appendToResponse = (isset($appendToResponse)) ? $appendToResponse : $this->getConfig()->getAppender('tvshow');

		return new TVShow($this->_call3('tv/' . $idTVShow, $appendToResponse, $language));
	}
	 ...
	 * 	@return Season
	 */
	public function getSeason($idTVShow, $numSeason, $appendToResponse = null, $language) {
		$appendToResponse = (isset($appendToResponse)) ? $appendToResponse : $this->getConfig()->getAppender('season');

		return new Season($this->_call3('tv/'. $idTVShow .'/season/' . $numSeason, $appendToResponse, $language), $idTVShow);
	}
	 ...
	 * 	@return Episode
	 */
	public function getEpisode($idTVShow, $numSeason, $numEpisode, $appendToResponse = null, $language) {
		$appendToResponse = (isset($appendToResponse)) ? $appendToResponse : $this->getConfig()->getAppender('episode');

		return new Episode($this->_call3('tv/'. $idTVShow .'/season/'. $numSeason .'/episode/'. $numEpisode, $appendToResponse, $language), $idTVShow);
	}
	 ...
	 * 	@return Person
	 */
	public function getPerson($idPerson, $appendToResponse = null, $language) {
		$appendToResponse = (isset($appendToResponse)) ? $appendToResponse : $this->getConfig()->getAppender('person');

		return new Person($this->_call3('person/' . $idPerson, $appendToResponse, $language));
	}
	 ...
	 * 	@return Movie[]
	 */
	public function searchMovie($movieTitle, $language){

		$movies = array();

		$result = $this->_call3('search/movie', '&query='. urlencode($movieTitle), $language);
	...
	private function _call3($action, $appendToResponse = '', $language) {

		$url = self::_API_URL_.$action .'?api_key='. $this->getConfig()->getAPIKey() .'&language='. $language .'&append_to_response='. implode(',', (array) $appendToResponse) .'&include_adult='. $this->getConfig()->getAdult();
	 ...
	 * 	@return TVShow[]
	 */
	public function searchTVShow($tvShowTitle, $language){

		$tvShows = array();

		$result = $this->_call('search/tv', '&query='. urlencode($tvShowTitle), $language);
	...
	public function getDiscoverMovies($page = 1, $year, $genre, $productor, $actor, $language) {

		$movies = array();

		$result = $this->_call2('discover/movie', '&page='. $page, $year, $genre, $productor, $actor, $language);
	...
	public function getDiscoverMovies2($page = 1, $year, $genre, $productor, $actor, $language) {

		$movies = array();

		$result = $this->_call5('discover/movie', '&page='. $page, $year, $genre, $productor, $actor, $language);
	...
	public function getDiscoverTVShows2($page = 1, $year, $genre, $language) {

		$tvShows = array();

		$result = $this->_call4('discover/tv', '&page='. $page, $year, $genre, $language);
	...
	private function _call2($action, $appendToResponse = '', $year, $genre, $productor, $actor, $language) {

		$url = self::_API_URL_.$action .'?api_key='. $this->getConfig()->getAPIKey() .'&language='. $language . '&primary_release_year=' . $year . '&sort_by=popularity.desc&vote_count.gte=50&with_genres=' . $genre . '&with_crew=' . $productor .'&with_cast=' . $actor .  '&append_to_response='. implode(',', (array) $appendToResponse) .'&include_adult='. $this->getConfig()->getAdult();
	...
	private function _call5($action, $appendToResponse = '', $year, $genre, $productor, $actor, $language) {

		$url = self::_API_URL_.$action .'?api_key='. $this->getConfig()->getAPIKey() .'&language='. $language . '&primary_release_year=' . $year . '&sort_by=popularity.desc&vote_count.gte=0&with_genres=' . $genre . '&with_crew=' . $productor .'&with_cast=' . $actor .  '&append_to_response='. implode(',', (array) $appendToResponse) .'&include_adult='. $this->getConfig()->getAdult();
	...
	private function _call4($action, $appendToResponse = '', $year, $genre,$language) {

		$url = self::_API_URL_.$action .'?api_key='. $this->getConfig()->getAPIKey() .'&language='. $language . '&first_air_date_year=' . $year . '&sort_by=popularity.desc&vote_count.gte=0&with_genres=' . $genre . '&append_to_response='. implode(',', (array) $appendToResponse) .'&include_adult='. $this->getConfig()->getAdult();
		//echo $url;
