...
<form style="text-align: center;"  method="post" action="index.php?zone=movies&item=discoverTV&language=<?php echo $_GET['language']; ?>">
...
//ini_set("display_errors",0);error_reporting(0);
	...
	if($year!=null || $genre!=null || $productor!=null || $actor!=null || $actor_id!=null || $director_id!=null)
	{
		$movies = $tmdb->getDiscoverTVShows2($page, $year, $genre_id, $director_id, $actor_id, $_GET['language']);
		?><div class='cadre'><?php 
		...
		?></div><?php 
		echo '          </ul>
		            </div> <a href="index.php?zone=movies&item=discoverTV&page=' . $page . '&year=' . $year . '&genre=' . $genre_id . '&productor=' . $director_id . '&actor=' . $actor_id  . '&language=' . $_GET['language'] . '"style="margin-left:85%"
